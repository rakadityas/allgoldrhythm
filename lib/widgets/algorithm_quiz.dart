...
class AlgorithmQuiz extends StatefulWidget {
  final Algorithm algorithm;

  const AlgorithmQuiz({super.key, required this.algorithm});

  @override
  State<AlgorithmQuiz> createState() => _AlgorithmQuizState();
}

class _AlgorithmQuizState extends State<AlgorithmQuiz> {
  ...
  void initState() {
    super.initState();
    _questions = QuizData.questionsFor(widget.algorithm.id);
  }
    ...
    if (_questions.isEmpty) {
      return Card(
        child: Padding(
          ...
          child: Text(
            'No quiz questions available yet for ${widget.algorithm.name}.',
  ...
  Widget _buildQuestion(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            Text('Question ${_index + 1} of ${_questions.length}', style: theme.textTheme.labelMedium),
            const Spacer(),
        ...
        if (answered) ...[
          ...
          Align(
            ...
            child: FilledButton.icon(
              onPressed: _next,
              icon: Icon(_index < _questions.length - 1 ? Icons.arrow_forward : Icons.flag),
              label: Text(_index < _questions.length - 1 ? 'Next' : 'See Results'),
            ),
          ),
        ],
      ],
